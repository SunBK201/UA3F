...
mkdir -p \
    ...
    $opkg_template/www/luci-static/resources/view/ua3f \
...
cp openwrt/files/ua3f.conf $opkg_template/etc/config/ua3f
cp openwrt/files/uci-defaults $opkg_template/etc/uci-defaults/luci-ua3f
cp -r openwrt/luasrc/* $opkg_template/usr/lib/lua/luci/
cp -r openwrt/htdocs/* $opkg_template/
