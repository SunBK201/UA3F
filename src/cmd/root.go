...
func init() {
	...
	viper.BindPFlag("bind-address", rootCmd.Flags().Lookup("bind"))
	viper.BindPFlag("port", rootCmd.Flags().Lookup("port"))
	viper.BindPFlag("log-level", rootCmd.Flags().Lookup("log-level"))
	viper.BindPFlag("user-agent", rootCmd.Flags().Lookup("ua"))
	viper.BindPFlag("user-agent-regex", rootCmd.Flags().Lookup("ua-regex"))
	viper.BindPFlag("user-agent-partial-replace", rootCmd.Flags().Lookup("partial"))
	viper.BindPFlag("rewrite-mode", rootCmd.Flags().Lookup("rewrite-mode"))
	viper.BindPFlag("header-rewrite-json", rootCmd.Flags().Lookup("header-rewrite"))
	viper.BindPFlag("body-rewrite-json", rootCmd.Flags().Lookup("body-rewrite"))
	viper.BindPFlag("url-redirect-json", rootCmd.Flags().Lookup("url-redirect"))
	viper.BindPFlag("ttl", rootCmd.Flags().Lookup("ttl"))
	viper.BindPFlag("ipid", rootCmd.Flags().Lookup("ipid"))
	viper.BindPFlag("tcp_timestamp", rootCmd.Flags().Lookup("tcpts"))
	viper.BindPFlag("tcp_initial_window", rootCmd.Flags().Lookup("tcpwin"))
	viper.BindPFlag("desync.reorder", rootCmd.Flags().Lookup("desync-reorder"))
	viper.BindPFlag("desync.reorder-bytes", rootCmd.Flags().Lookup("desync-reorder-bytes"))
	viper.BindPFlag("desync.reorder-packets", rootCmd.Flags().Lookup("desync-reorder-packets"))
	viper.BindPFlag("desync.inject", rootCmd.Flags().Lookup("desync-inject"))
	viper.BindPFlag("desync.inject-ttl", rootCmd.Flags().Lookup("desync-inject-ttl"))
	viper.BindPFlag("desync.desync-ports", rootCmd.Flags().Lookup("desync-ports"))
	...
	viper.BindEnv("server-mode", "UA3F_SERVER_MODE")
	viper.BindEnv("bind-address", "UA3F_BIND_ADDRESS")
	viper.BindEnv("port", "UA3F_PORT")
	viper.BindEnv("log-level", "UA3F_LOG_LEVEL")
	viper.BindEnv("rewrite-mode", "UA3F_REWRITE_MODE")
	viper.BindEnv("user-agent", "UA3F_PAYLOAD_UA")
	viper.BindEnv("user-agent-regex", "UA3F_UA_REGEX")
	viper.BindEnv("user-agent-partial-replace", "UA3F_PARTIAL_REPLACE")
	viper.BindEnv("ttl", "UA3F_TTL")
	viper.BindEnv("ipid", "UA3F_IPID")
	viper.BindEnv("tcp_timestamp", "UA3F_TCPTS")
	viper.BindEnv("tcp_initial_window", "UA3F_TCP_INIT_WINDOW")
	viper.BindEnv("desync.reorder", "UA3F_DESYNC_REORDER")
	viper.BindEnv("desync.reorder-bytes", "UA3F_DESYNC_REORDER_BYTES")
	viper.BindEnv("desync.reorder-packets", "UA3F_DESYNC_REORDER_PACKETS")
	viper.BindEnv("desync.inject", "UA3F_DESYNC_INJECT")
	viper.BindEnv("desync.inject-ttl", "UA3F_DESYNC_INJECT_TTL")
	viper.BindEnv("desync.desync-ports", "UA3F_DESYNC_PORTS")
	viper.BindEnv("header-rewrite-json", "UA3F_HEADER_REWRITE")
	viper.BindEnv("body-rewrite-json", "UA3F_BODY_REWRITE")
	viper.BindEnv("url-redirect-json", "UA3F_URL_REDIRECT")
}
